...
gsutil -m cp "$BUCKET/results/*.json" "$LOCAL_RESULTS\" 2>$null
if ($LASTEXITCODE -eq 0) {
    Write-Host "  Done"
} else {
    Write-Warning "No result files found yet"
}

# Download logs
Write-Host "[2/3] Downloading training logs..."
gsutil -m cp "$BUCKET/logs/*.log" "$LOCAL_LOGS\" 2>$null
if ($LASTEXITCODE -eq 0) {
    Write-Host "  Done"
} else {
    Write-Warning "No log files found yet"
}
